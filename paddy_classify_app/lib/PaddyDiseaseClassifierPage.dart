  ...
  Future<void> _classifyPaddyDisease() async {
    ...
    try {
      ...
      var request = http.MultipartRequest(
        ...
        Uri.parse(
            'https://ml-disease-classifier-production.up.railway.app/predict'),
      );

      // Add paddy image to the request
      request.files.add(
        await http.MultipartFile.fromPath('file', _paddyImageFile!.path),
      );
      ...
      if (response.statusCode == 200) {
        ...
        final predictionIndex = int.parse(data['prediction']); // "3" -> 3
        final diseaseName = _categories[predictionIndex]; // "FalseMut"

        final probs = Map<String, dynamic>.from(data['probabilities']);
        final confidence = probs[predictionIndex.toString()]; // 0.4933
